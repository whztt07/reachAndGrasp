 ...
 Sample_FacialAnimation():
    ...
    bool frameRenderingQueued(const FrameEvent& evt)
    {
        ...
        if (mPlayAnimation)
        {
            for(int i=0;i<AnimNum;i++)
            {
                mSpeakAnimState2[i]->addTime(evt.timeSinceLastFrame);
            }
        }
 ...
 protected:
    ...
    void setupContent(void)
    {
        ...
        for(int i=0;i<AnimNum;i++)
        {
            char temp[10];
            sprintf(temp, "%d",i);
            Entity* head2 = mSceneMgr->createEntity(Ogre::String("Head")+temp, "facial.mesh");
            mSceneMgr->getRootSceneNode()->createChildSceneNode(Vector3(10*i, -30, 0))->attachObject(head2);
            mSpeakAnimState2[i] = head2->getAnimationState("Speak");
        }
    ...
    void checkBoxToggled(OgreBites::CheckBox * box)
    {
        ...
        for(int i=0;i<AnimNum;i++)
        {
            mSpeakAnimState2[i]->setEnabled(mPlayAnimation);
        }
    ...
    AnimationState* mSpeakAnimState2[AnimNum];
