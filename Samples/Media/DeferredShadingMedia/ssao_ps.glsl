...
void main()
{
    ...
    vec3 randN = texture2DLod(randMap, oUv0 * 24, 0).xyz * 2.0 - 1.0;
    vec4 geom = texture2DLod(geomMap, oUv0, 0);
    float depth = geom.w;
    ...
    for (int i = 0; i < NUM_BASE_SAMPLES; ++i)
    {
        ...
        float zd = clamp(far * (depth - texture2DLod(geomMap, nuv.xy, nuv.w).w), 0.0, 1.0);
