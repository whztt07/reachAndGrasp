...
void main()
{
   ...
   vec3 lightDir_1 = normalize(lightPosition_1.xyz - (vertex * lightPosition_1.w).xyz);
   vec3 lightDir_2 = normalize(lightPosition_2.xyz - (vertex * lightPosition_2.w).xyz);
   ...
   v_TSLightDir_1 = rotation * lightDir_1;
   v_TSLightDir_2 = rotation * lightDir_2;
   v_TSViewDir = rotation * cameraDir;

   vec3 eyeDir = normalize(eyePosition.xyz - vertex.xyz);
   vec3 halfAngle = normalize(eyeDir + lightDir);
   vec3 halfAngle_1 = normalize(eyeDir + lightDir_1);
   vec3 halfAngle_2 = normalize(eyeDir + lightDir_2);
   v_TSHalfAngle = rotation * halfAngle;
   v_TSHalfAngle_1 = rotation * halfAngle_1;
   v_TSHalfAngle_2 = rotation * halfAngle_2;
   v_TSPos = rotation * vertex.xyz;  

   v_TSNormal = rotation * normal;
   v_TSTangent = rotation * tangent;
#else
    gl_Position = worldViewProj * vertex;//gl_Vertex;
    v_norm = normal;
    v_vertex = vertex;
